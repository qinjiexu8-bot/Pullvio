...
export default function AuthForm({ mode, locale }: { mode: Mode; locale: Locale }) {
  ...
  async function submit(event: FormEvent) {
    ...
    if (mode !== "forgot" && !password) return fail("password", t.passwordRequired);
    if ((mode === "signup" || mode === "reset") && password.length < 8) return fail("password", t.short);
    if ((mode === "signup" || mode === "reset") && !confirmPassword) return fail("confirmPassword", t.confirmRequired);
    if ((mode === "signup" || mode === "reset") && password !== confirmPassword) return fail("confirmPassword", t.mismatch);
    const supabase = createClient();
    ...
    try {
      if (mode === "login") {
        ...
        const { error } = await supabase.auth.signUp({ email: normalizedEmail, password, options: { data: { locale }, emailRedirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent(localePath(locale, "/account"))}` } });
        if (error) throw error; setSuccess(true); setMessage(t.checkEmail);
      } else if (mode === "forgot") {
        const { error } = await supabase.auth.resetPasswordForEmail(normalizedEmail, { redirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent(localePath(locale, "/reset-password"))}` });
