...
export default function ContentPage({ eyebrow, title, description, updated, afterHeader, children, links, locale = "en" }: { eyebrow: string; title: string; description: string; updated?: string; afterHeader?: ReactNode; children: ReactNode; links?: Array<[string, string]>; locale?: Locale }) {
  const labels = pageLabels[locale];
  return <main className="content-page"><SiteHeader locale={locale} simple /><div className="shell content-page-main"><header className="content-hero"><span className="kicker">{eyebrow}</span><h1>{title}</h1><p>{description}</p>{updated && <div className="content-meta"><span>{labels.updated} {updated}</span><span>{labels.context}</span></div>}{afterHeader}</header><article className="content-body">{children}{links && <nav className="content-links" aria-label={labels.related}>{links.map(([label, href]) => <Link href={href} key={href}>{label}<ArrowRight size={16} /></Link>)}</nav>}</article></div><SiteFooter locale={locale} /></main>;
}
