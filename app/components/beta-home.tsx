...
export default function BetaHome({ locale }: { locale: Locale }) {
  ...
  return <main id="top">
    ...
    <section className="guides-preview" id="guides"><div className="shell guides-head"><div><span className="kicker">{t.guidesKicker}</span><h2>{t.guidesTitle}</h2></div><p>{t.guidesCopy}</p></div><div className="shell guide-preview-grid">{t.guides.map(([title, copy, href], index) => <Link className="guide-preview-card" href={localePath(locale, href)} key={href}><div>{index === 0 ? <FileAudio size={23} /> : index === 1 ? <FileVideo2 size={23} /> : <ShieldCheck size={23} />}<span>GUIDE 0{index + 1}</span></div><h3>{title}</h3><p>{copy}</p><b>{String(t.guideCta)} <ArrowRight size={15} /></b></Link>)}</div></section>
