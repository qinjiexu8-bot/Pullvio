...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const localizedPaths = ["/about", "/guides", ...guides.map(({ slug }) => `/guides/${slug}`), "/blog", ...blogPosts.map(({ slug }) => `/blog/${slug}`)];
  const localizedPages: MetadataRoute.Sitemap = localizedPaths.flatMap((path) => {
    ...
    return ["", "/zh-cn", "/es"].map((locale) => ({
      ...
      lastModified,
      changeFrequency: path === "/about" ? "yearly" as const : path === "/blog" ? "weekly" as const : "monthly" as const,
      priority: path === "/guides" || path === "/blog" ? 0.8 : path === "/about" ? 0.4 : 0.75,
